 ...
 * Configure VITE_API_BASE at build time for a deployed backend.
 */
...
function switchTab(tabId) {
  ...
  const titles = {
    ...
    'kisanchat': 'किसान चैट — गाँव समूह (Kisan Chat · P2P Village Group)',
...
/* ─── 12. Kisan Chat (P2P Farmer-to-Farmer, offline) ───
   Mirrors the Flutter KisanChatService exactly: same seed conversation,
   same PEERS, same keyword replyFor() logic, and the same offline store
   key 'kisan_chat_messages_v1'. Self-contained IIFE — no globals leak. */
(function () {
  const STORAGE_KEY = 'kisan_chat_messages_v1';
  const COMMUNITY_KEY = 'kisan_communities_v1';
  const DEFAULT_COMMUNITY = 'village-sonipat';
  const ME_ID = 'rameshwar-singh';
  const ME_NAME = 'Rameshwar Singh';
  const PEERS = [
  ...
  let communities = [];
  let messages = [];
  let activeCommunityId = DEFAULT_COMMUNITY;
  let peerTyping = false;

  function seedCommunities() {
    return [
      { id: 'village-sonipat', name: 'सोनीपत गाँव समूह', topic: 'सामान्य चर्चा · गेहूँ/सरसों', emoji: '🌾', createdBy: 'Jagbir Singh',  memberCount: 12, joined: true  },
      { id: 'wheat-disease',   name: 'गेहूँ रोग चर्चा',   topic: 'रोग व उपचार',              emoji: '🦠', createdBy: 'Suresh Yadav',  memberCount: 38, joined: false },
      { id: 'mandi-bhav',      name: 'मंडी भाव अपडेट',    topic: 'रोज़ के दाम',              emoji: '📈', createdBy: 'Mukesh Sharma', memberCount: 54, joined: false },
      { id: 'organic',         name: 'जैविक खेती',        topic: 'ऑर्गैनिक तरीके',           emoji: '🌱', createdBy: 'Rajbir Dhaka',  memberCount: 21, joined: false },
    ];
  }

  function seedConversation() {
    const now = Date.now(); const min = 60000;
    const m = (minAgo, cid, id, name, text, me = false) =>
      ({ id: `seed-${cid}-${id}-${minAgo}`, communityId: cid, senderId: id, senderName: name, text,
         timestamp: new Date(now - minAgo * min).toISOString(), isMe: me });
    return [
      m(180, 'village-sonipat', 'jagbir-singh', 'Jagbir Singh', 'Ram-Ram kisaan bhaiyo! Mera gehu 45 din ka ho gaya. Kal halki sinchai ki thi, nami badhiya ban gayi.'),
      m(150, 'village-sonipat', 'mukesh-sharma', 'Mukesh Sharma', 'Ram-Ram! Rai mandi mein aaj sarson ₹5,450/qtl chal raha hai. Kisaan bhai dhyan dein.'),
      m(90,  'village-sonipat', 'suresh-yadav', 'Suresh Yadav', 'Bhaiyo, mere gehu ki patti par pile dhabbe aa rahe hain. Kya yeh pila ratua hai?'),
      m(80,  'village-sonipat', 'rameshwar-singh', 'Rameshwar Singh', 'Haan Suresh bhai, lagta to pila ratua (Yellow Rust) hi hai. Kaunsi kism boyi thi?', true),
      m(60,  'village-sonipat', 'jagbir-singh', 'Jagbir Singh', 'Propiconazole 25% EC @ 1ml/litre pani spray karo — 200ml prati acre. 15 din baad dobara. HD-3086 rog-rodhak kism hai.'),
      m(200, 'wheat-disease', 'suresh-yadav', 'Suresh Yadav', 'Is group mein sirf rog aur upchar ki baat karein. Patti ki photo bhi daal sakte ho.'),
      m(120, 'wheat-disease', 'jagbir-singh', 'Jagbir Singh', 'Pila ratua ke liye Propiconazole sabse asardar hai. Subah-subah spray karo.'),
      m(240, 'mandi-bhav', 'mukesh-sharma', 'Mukesh Sharma', 'Aaj ke bhav — Sarson ₹5,450, Gehu ₹2,275, Chana ₹5,100 prati qtl.'),
      m(60,  'mandi-bhav', 'jagbir-singh', 'Jagbir Singh', 'Sonipat mandi mein gehu ka rate aaj thoda upar hai.'),
      m(300, 'organic', 'rajbir-dhaka', 'Rajbir Dhaka', 'Jeevamrut aur neem khali se acchi paidawar mil rahi hai. Rasayan kam karo bhaiyo.'),
    ];
  }

  function persist() { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(messages)); } catch (e) {} }
  function persistCommunities() { try { localStorage.setItem(COMMUNITY_KEY, JSON.stringify(communities)); } catch (e) {} }

  function loadCommunities() {
    try { const raw = localStorage.getItem(COMMUNITY_KEY); if (raw) communities = JSON.parse(raw); }
    catch (e) { communities = []; }
    if (!Array.isArray(communities) || communities.length === 0) { communities = seedCommunities(); persistCommunities(); }
  }

  function load() {
    try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) messages = JSON.parse(raw); }
    catch (e) { messages = []; }
    if (!Array.isArray(messages) || messages.length === 0) { messages = seedConversation(); persist(); }
    // Migration: messages saved before communities default to the village group.
    let changed = false;
    for (const m of messages) { if (!m.communityId) { m.communityId = DEFAULT_COMMUNITY; changed = true; } }
    if (changed) persist();
  }

  function communityGet(id) { return communities.find(c => c.id === id); }
  function communitiesJoined() { return communities.filter(c => c.joined); }
  function communitiesDiscover() { return communities.filter(c => !c.joined); }
  function messagesFor(id) { return messages.filter(m => m.communityId === id); }

  function createCommunity(name, topic, emoji) {
    const id = 'c-' + Date.now();
    communities.unshift({ id, name, topic: topic || 'नया समुदाय', emoji: emoji || '🌾', createdBy: ME_NAME, memberCount: 1, joined: true });
    persistCommunities();
    messages.push({ id: String(Date.now()), communityId: id, senderId: ME_ID, senderName: ME_NAME,
      text: `नमस्ते! Maine "${name}" samuday banaya hai. Kisaan bhai judein aur charcha karein. 🌾`,
      timestamp: new Date().toISOString(), isMe: true });
    persist();
    return id;
  }
  function joinCommunity(id) {
    const c = communityGet(id);
    if (c && !c.joined) { c.joined = true; c.memberCount += 1; persistCommunities(); }
  }

  function replyFor(text) {
    const q = (text || '').toLowerCase();
    if (q.includes('रतुआ') || q.includes('rust') || q.includes('पीला') || q.includes('gehu') || q.includes('गेहूं') || q.includes('wheat'))
    ...
    if (q.includes('धान') || q.includes('rice') || q.includes('झुलसा') || q.includes('blast') || q.includes('paddy'))
      return 'Dhan ke jhulsa ke liye Tricyclazole 75% WP @ 0.6g/litre spray karo. Khet se extra pani nikal do.';
    if (q.includes('कपास') || q.includes('cotton') || q.includes('सुंडी') || q.includes('bollworm'))
      return 'Gulabi sundi ke liye pheromone trap lagao — 8 prati acre. Emamectin benzoate 5% SG bhi asardar hai.';
    if (q.includes('bhav') || q.includes('भाव') || q.includes('mandi') || q.includes('मंडी') || q.includes('price') || q.includes('rate'))
      return 'Aaj mandi mein bhav theek chal raha hai. Subah jaldi lekar jao to behtar rate milta hai.';
    if (q.includes('मौसम') || q.includes('weather') || q.includes('barish') || q.includes('बारिश') || q.includes('rain'))
      return 'Agle 2 din halki barish ka anuman hai. Spray karna hai to aaj hi kar lo.';
    if (q.includes('ram-ram') || q.includes('namaste') || q.includes('नमस्ते') || q.includes('hello') || q.includes('hi '))
  ...
  function initials(name) {
    ...
    return (p.length >= 2 ? (p[0][0] + p[1][0]) : ((name || '?')[0] || '?')).toUpperCase();
  }
  function fmtTime(iso) {
    const t = new Date(iso); let h = t.getHours() % 12; if (h === 0) h = 12;
    const mm = String(t.getMinutes()).padStart(2, '0');
    return `${h}:${mm} ${t.getHours() < 12 ? 'AM' : 'PM'}`;
  }
  function esc(s) { return (s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

  function bubbleHTML(msg) {
    const mine = msg.isMe;
    const av = `<div style="width:30px;height:30px;border-radius:50%;flex:0 0 auto;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;border:1px solid var(--border-glass);${mine ? 'background:#065F46;color:#D1FAE5;' : 'background:rgba(16,185,129,0.12);color:#6EE7B7;'}">${initials(msg.senderName)}</div>`;
    const name = mine ? '' : `<div style="font-size:11px;font-weight:800;color:#6EE7B7;margin-bottom:2px;">${esc(msg.senderName)}</div>`;
    const check = mine ? ' <span style="color:#A7F3D0;">✓✓</span>' : '';
    const bub = `<div style="max-width:74%;padding:8px 12px;border-radius:14px;font-size:13px;line-height:1.4;${mine ? 'background:#059669;color:#ffffff;border-bottom-right-radius:4px;' : 'background:rgba(255,255,255,0.06);color:var(--text-primary);border:1px solid var(--border-glass);border-bottom-left-radius:4px;'}">${name}<div>${esc(msg.text)}</div><div style="font-size:9.5px;margin-top:3px;${mine ? 'color:rgba(255,255,255,0.75);text-align:right;' : 'color:var(--text-tertiary);'}">${fmtTime(msg.timestamp)}${check}</div></div>`;
    return `<div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:6px;${mine ? 'justify-content:flex-end;' : ''}">${mine ? bub + av : av + bub}</div>`;
  }

  function renderCommunities() {
    const bar = document.getElementById('kisanchat-communities');
    if (bar) {
      bar.innerHTML = communitiesJoined().map(c => {
        const active = c.id === activeCommunityId;
        return `<button class="badge-tag ${active ? 'status-live' : ''}" data-cid="${c.id}" title="${esc(c.topic)}" style="cursor:pointer;border:1px solid var(--border-glass);${active ? '' : 'opacity:.65;'}">${c.emoji} ${esc(c.name)} · ${c.memberCount}</button>`;
      }).join('');
      bar.querySelectorAll('[data-cid]').forEach(b => b.addEventListener('click', () => setActive(b.getAttribute('data-cid'))));
    }
    renderDiscover();
  }

  function renderDiscover() {
    const row = document.getElementById('kisanchat-discover');
    if (!row) return;
    const disc = communitiesDiscover();
    if (disc.length === 0) { row.innerHTML = ''; return; }
    row.innerHTML = `<span style="font-size:11px;color:var(--text-tertiary);">शामिल हों (किसानों द्वारा बनाए):</span>` +
      disc.map(c => `<button class="btn-ghost" data-join="${c.id}" title="बनाया: ${esc(c.createdBy)}" style="font-size:11px;">${c.emoji} ${esc(c.name)} ＋</button>`).join('');
    row.querySelectorAll('[data-join]').forEach(b => b.addEventListener('click', () => { joinCommunity(b.getAttribute('data-join')); renderCommunities(); }));
  }

  function setActive(id) { activeCommunityId = id; peerTyping = false; renderCommunities(); render(); }

  function render() {
    const thread = document.getElementById('kisanchat-thread');
    if (!thread) return;
    const c = communityGet(activeCommunityId);
    const tag = document.getElementById('kisanchat-online-tag');
    if (tag && c) tag.textContent = `🟢 ${c.name} · ${c.memberCount} सदस्य`;
    const list = messagesFor(activeCommunityId);
    let html = list.map(bubbleHTML).join('');
    if (list.length === 0 && !peerTyping) html = `<div style="margin:auto;color:var(--text-tertiary);font-size:13px;text-align:center;">अभी कोई संदेश नहीं — पहला संदेश भेजकर charcha shuru karein 🌾</div>`;
    if (peerTyping) html += `<div style="display:flex;margin-bottom:6px;"><div style="padding:9px 14px;border-radius:14px;font-size:12px;font-style:italic;color:var(--text-tertiary);background:rgba(255,255,255,0.06);border:1px solid var(--border-glass);">कोई किसान लिख रहा है…</div></div>`;
    thread.innerHTML = html;
    thread.scrollTop = thread.scrollHeight;
  }

  function send() {
    const input = document.getElementById('kisanchat-input');
    if (!input) return;
    const text = input.value.trim();
    if (!text) return;
    input.value = '';
    const cid = activeCommunityId;
    messages.push({ id: String(Date.now()), communityId: cid, senderId: ME_ID, senderName: ME_NAME, text, timestamp: new Date().toISOString(), isMe: true });
    persist();
    peerTyping = true;
    render();
    setTimeout(() => {
      const peer = PEERS[new Date().getSeconds() % PEERS.length];
      messages.push({ id: String(Date.now()), communityId: cid, senderId: peer[0], senderName: peer[1], text: replyFor(text), timestamp: new Date().toISOString(), isMe: false });
      peerTyping = false;
      persist();
      if (activeCommunityId === cid) render();
    }, 1300);
  }

  function clearChat() {
    messages = messages.filter(m => m.communityId !== activeCommunityId);
    persist();
    render();
    renderCommunities();
  }
  ...
  function toggleCreate(show) {
    const f = document.getElementById('kisanchat-create');
    if (f) f.style.display = show ? 'flex' : 'none';
  }
  function doCreate() {
    const nameEl = document.getElementById('kisanchat-new-name');
    const topicEl = document.getElementById('kisanchat-new-topic');
    const emojiEl = document.getElementById('kisanchat-new-emoji');
    if (!nameEl) return;
    const name = nameEl.value.trim();
    if (!name) { nameEl.focus(); return; }
    const id = createCommunity(name, topicEl ? topicEl.value.trim() : '', emojiEl ? emojiEl.value : '🌾');
    nameEl.value = ''; if (topicEl) topicEl.value = '';
    toggleCreate(false);
    setActive(id);
  }
  ...
  function init() {
    ...
    renderCommunities();
    render();
    document.getElementById('btn-send-kisanchat')?.addEventListener('click', send);
    document.getElementById('kisanchat-input')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') send(); });
    document.getElementById('btn-clear-kisanchat')?.addEventListener('click', clearChat);
    document.getElementById('btn-new-community')?.addEventListener('click', () => toggleCreate(true));
    document.getElementById('btn-cancel-community')?.addEventListener('click', () => toggleCreate(false));
    document.getElementById('btn-create-community')?.addEventListener('click', doCreate);
    document.getElementById('kisanchat-new-name')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') doCreate(); });
    // Re-scroll to newest message whenever the Kisan Chat tab is opened
    document.getElementById('btn-tab-kisanchat')?.addEventListener('click', () => setTimeout(render, 60));
  }
